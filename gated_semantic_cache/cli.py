...
from gated_semantic_cache.eval.offline_benchmark import benchmark_routing_classifier, inspect_queries
from gated_semantic_cache.eval.shadow_compare import run_shadow_compare
from gated_semantic_cache.eval.queries_regression import run_queries_regression_report
from gated_semantic_cache.eval.structured_extract_eval import run_legacy_structured_coverage
...
from gated_semantic_cache.models.context import DEFAULT_MAX_PRIOR_USER_QUERIES, DEFAULT_SEMANTIC_LOW_WATERMARK, RequestContext
...
from gated_semantic_cache.serving.pipeline import SemanticCachePipeline
...
def _default_persistent_db_path() -> Path:
    raw = os.environ.get("GATED_SEMANTIC_CACHE_DB")
    if raw is not None and str(raw).strip():
        return Path(raw).expanduser()
    return Path.cwd() / ".gated-semantic-cache" / "cache.sqlite3"
...
def _add_cache_shared_core(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="SQLite path (default: $GATED_SEMANTIC_CACHE_DB or .gated-semantic-cache/cache.sqlite3)",
    )
